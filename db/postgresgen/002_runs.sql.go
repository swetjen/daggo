...
WHERE ($1 = 0 OR r.job_id = $1)
  ...
  AND (
    $5 = 0
    OR r.queued_at >= now() - ($5::bigint * interval '1 hour')
  )
`
...
JOIN jobs j ON j.id = r.job_id
WHERE r.job_id = $1
  AND COALESCE(r.started_at, r.queued_at) >= $2::timestamptz
  AND COALESCE(r.started_at, r.queued_at) <= $3::timestamptz
...
WHERE ($1 = 0 OR r.job_id = $1)
  ...
  AND (
    $6 = ''
    OR COALESCE(r.started_at, r.queued_at) < NULLIF($6, '')::timestamptz
    OR (COALESCE(r.started_at, r.queued_at) = NULLIF($6, '')::timestamptz AND r.id < $7)
  )
ORDER BY COALESCE(r.started_at, r.queued_at) DESC, r.id DESC
LIMIT $8
`
